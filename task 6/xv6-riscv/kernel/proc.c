...
fork(void)
{
  ...
  np->stime = 0;
  np->retime = 0;
  np->last_executed = p->last_executed;
  ...
  for(;;){
    ...
    if(min_vrun_proc != 0){
      ...
      if(min_vrun_proc->state == RUNNABLE){
        ...
        //Update retime (runnable time)
        min_vrun_proc->retime += (ticks - min_vrun_proc->last_executed);
...
{
  struct proc *p = myproc();
  // record the sleep start time
  uint64 sleep_start_ticks = ticks;
  ...
  p->stime += ( ticks - sleep_start_ticks );
